...
function crea_tableData() {
  var sql =
    "CREATE TABLE IF NOT EXISTS data(data_soggiorno date not null, ref_casa_o VARCHAR(50) references casa(id_casa), disponibilita boolean, primary key (data_soggiorno, ref_casa_o, disponibilita))";
  con.query(sql, function (err, result) {
    if (err) throw err;
